...
class PasswordChangeView(LoginRequeridoPerAuth, SuccessMessageMixin,
    ...
    success_url = '/inicio/'
    success_message = "Cambio de contraseña con éxito"
...
class RegisterView(LoginRequeridoPerAuth, MultiModelFormView):
    ...
    def forms_valid(self, forms, **kwargs):
        ...
        """
        nuevo_usuario = forms['user'].save(commit=False)
        ...
        for group in grupos:
            ...
            if group.name == "Voceros":
                documento_identidad = nuevo_perfil.id_perfil
                tipo_documento = nuevo_perfil.fk_tipo_documento
                organizacion_social = forms['user_perfil'].cleaned_data['organizacion']
                vocero = Vocero.objects.update_or_create(fk_org_social=organizacion_social,
                    fk_tipo_documento=tipo_documento,
                    documento_identidad=documento_identidad,
                    defaults={
                        'nombres': nuevo_usuario.first_name,
                        'apellidos': nuevo_usuario.last_name
                    }
                )[0]
                # Asocia el vocero con la nueva cuenta de usuario
                asociar_voceros = UserProfileVocero(fk_user=nuevo_usuario,
                    fk_vocero=vocero)
        ...
        messages.success(self.request, "Usuario %s creado con éxito\
                                       " % (str(usuario)))
...
class DataDetailView(LoginRequeridoPerAuth, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        @return: El contexto con los objectos para la vista
        """
        print("Entra primero a get_context")
...
class UpdatePerfilAdmin(LoginRequeridoPerAuth, MultiModelFormView):
    ...
    def forms_valid(self, forms, **kwargs):
        ...
        if self.record_id is not None:
            messages.success(self.request, "Usuario %s Actualizado con éxito\
                                           " % (str(objeto.username)))
...
class RegisterVocerosView(LoginRequeridoPerAuth, MultiModelFormView):
    ...
    def forms_valid(self, forms, **kwargs):
        ...
        @return: Dirige con un mensaje de éxito al registro de proyecto
        """
